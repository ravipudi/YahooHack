...
while($row = mysql_fetch_array($hashtags))
  {
	 $topic_string=$topic_string."<div onclick='fetch()' id='tags'>".$row['h_text']."</div>";
}
echo "SELECT  `p_text`,`p_url` FROM `scribble_db`.`posts` WHERE `u_id`='".$uid."' and `p_text` LIKE '%".$e."%';" ;
$posts=mysql_query("SELECT  `p_text`,`p_url` FROM `scribble_db`.`posts` WHERE `u_id`='".$uid."' and `p_text` LIKE '%".$e."%';");
$post_string="";
